# -*- coding: utf-8 -*-

from . import import_inventory_wizard
from . import import_excel_wizard
from . import fix_locations_wizard
from . import import_method_wizard
from . import import_kobo_wizard
from . import initial_stock_wizard
from . import fix_product_types_wizard
from . import cancel_inventory_wizard
from . import stock_data_cleanup_wizard
from . import import_flexible_inventory_wizard
from . import stock_accounts_config_wizard
from . import warehouse_valuation_export_wizard
